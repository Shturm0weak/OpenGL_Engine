...
class Coin: public GameObject, public Listener{
public:
	...
	Coin(const std::string name = "Coin", float x = 0, float y = 30) :GameObject(name, x, y) {
		...
		*Renderer2DLayer::type = "GameObject";
		...
		col->IsTrigger = true;
		col->SetTag("Coin");
		prevscale[0] = scaleinfo[0];
		prevscale[1] = scaleinfo[1];
	}
	...
	virtual void OnUpdate() override {
		if (playanim) {
			...
			if (!secondphase) {
				tr->Scale(scaleinfo[0] * 1.04f, scaleinfo[1] * 1.04f);
				if (scaleinfo[0] > prevscale[0] * 1.5f)
					secondphase = true;
			}
			else if (secondphase) {
				tr->Scale(scaleinfo[0] * 0.95f, scaleinfo[1] * 0.95f);
			}

			if (scaleinfo[0] < prevscale[0] * 0.5f) {
		...
		col->IsCollidedDIAGS();
		if (col->_isCollided) {
			EventSystem::Instance()->SendEvent("OnCollision", this, col->_collidedObj);
		}
		tr->Move(0,-5,0);
		if (tr->position.y <= -5) {
			Randomize();
	...
	virtual void OnCollision(void* _col)override {
		...
		if (col->GetTag() == "Player") {
			...
			Player* player = dynamic_cast<Player*>(col->owner);
			player->scores++;
			player = nullptr;
			playanim = true;
		}
